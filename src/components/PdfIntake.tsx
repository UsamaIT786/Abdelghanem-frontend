...
import { fetchLiveDocuments, uploadLiveDocument, updateLiveDocument, deleteLiveDocument, initLiveWebSocket 
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setDeleteConfirm(null)}>
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 /50 rounded-2xl p-6 max-w-sm w-full mx-4 shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-sm text-slate-600 dark:text-slate-300 mb-6">Permanently delete <strong className="text-slate-900 dark:text-white ">{deleteConfirm.name}</strong>? This cannot be undone.</p>
            <div className="flex gap-2 justify-end">
              ...
              <button onClick={handleDeleteConfirm} className="px-4 py-2 rounded-xl bg-gradient-to-r from-rose-600 to-pink-600 hover:from-rose-500 hover:to-pink-500 text-white text-sm font-bold transition-all flex items-center gap-1.5"><Trash2 className="w-4 h-4" /> Delete</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-white dark:from-slate-900 via-slate-50 dark:via-slate-800 to-white dark:to-slate-900 p-6 md:p-8 shadow-2xl border border-slate-200 dark:border-slate-700 /50">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-indigo-400 to-cyan-500 flex items-center justify-center text-white shadow-xl shadow-indigo-500/30 ring-2 ring-white/10">
              <BrainCircuit className="w-7 h-7" />
            </div>
            <div>
              <h1 className="text-xl md:text-2xl font-bold text-slate-900 dark:text-white tracking-tight">AI Smart PDF Intake</h1>
              <p className="text-sm text-slate-600 dark:text-slate-300 mt-0.5">Programmatic extraction for tenders, quotes & engineering surveys</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {editingDoc && (
        <div className="rounded-2xl bg-gradient-to-br from-white dark:from-slate-900 to-slate-50 dark:to-slate-800 border border-slate-200 dark:border-slate-700 /50 p-6 shadow-xl animate-scale-in">
          <form onSubmit={handleSaveEditDoc} className="space-y-5">
            <div className="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-700 /50">
              <span className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2"><Pencil className="w-4 h-4 text-indigo-400" /> {editingDoc.fileName}</span>
              <button type="button" onClick={() => setEditingDoc(null)} className="btn-ghost text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white "><X className="w-4 h-4" /></button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="text-[10px] uppercase font-bold text-slate-600 dark:text-slate-300 mb-1.5 block">File Name</label>
              ...
              <div>
                <label className="text-[10px] uppercase font-bold text-slate-600 dark:text-slate-300 mb-1.5 block">Client Name</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="text-[10px] uppercase font-bold text-slate-600 dark:text-slate-300 mb-1.5 block">Address</label>
                <input type="text" className="w-full px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 /50 text-slate-900 dark:text-white text-sm focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/20 outline-none transition-all" value={editAddress} onChange={e => setEditAddress(e.target.value)} />
              </div>
              <div>
                <label className="text-[10px] uppercase font-bold text-slate-600 dark:text-slate-300 mb-1.5 block">Total Amount</label>
                <input type="text" className="w-full px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 /50 text-slate-900 dark:text-white text-sm focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/20 outline-none transition-all" value={editTotalAmount} onChange={e => setEditTotalAmount(e.target.value)} />
              </div>
            </div>
            <div>
              <label className="text-[10px] uppercase font-bold text-slate-600 dark:text-slate-300 mb-1.5 block">Items</label>
              <textarea className="w-full px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 /50 text-slate-900 dark:text-white text-sm h-20 focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/20 outline-none transition-all" value={editItems} onChange={e => setEditItems(e.target.value)} />
            </div>
            <div>
              <label className="text-[10px] uppercase font-bold text-slate-600 dark:text-slate-300 mb-1.5 block">Confidence %</label>
            ...
            <div className="flex gap-2 pt-2">
              <button type="submit" className="flex-1 px-5 py-2.5 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-400 hover:to-purple-500 text-white text-sm font-bold transition-all shadow-lg shadow-indigo-500/20 flex items-center justify-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 space-y-5">
          ...
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-white dark:from-slate-900 to-slate-50 dark:to-slate-800 border border-slate-200 dark:border-slate-700 /50 p-8 cursor-pointer group text-center hover:border-indigo-500/30 transition-all duration-300">
            ...
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-indigo-500/20 to-cyan-500/20 border border-indigo-500/10 flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform duration-300">
              <Upload className="w-7 h-7 text-indigo-400" />
            </div>
            <span className="text-base font-bold text-slate-900 dark:text-white block">Drop PDF or click to upload</span>
            <span className="text-xs text-slate-600 dark:text-slate-300 mt-1.5 block">Engineering quotes, tenders, surveys — up to 10MB</span>
            ...
            {isUploading && (
              <div className="absolute inset-0 bg-white dark:bg-slate-900 backdrop-blur-sm rounded-2xl flex flex-col justify-center items-center px-8 z-20">
                ...
                <div className="w-full max-w-xs bg-slate-50 dark:bg-slate-900/50 h-2.5 rounded-full overflow-hidden">
                  <div className="h-full rounded-full bg-gradient-to-r from-indigo-500 to-cyan-500 transition-all duration-300" style={{ width: `${uploadProgress}%` }} />
                </div>
              </div>
            )}
          </div>
          ...
          <div className="rounded-2xl bg-gradient-to-br from-white dark:from-slate-900 to-slate-50 dark:to-slate-800 border border-slate-200 dark:border-slate-700 /50 p-6 shadow-xl">
            <div className="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-700 /50 mb-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300 ">Document Ledger</span>
              </div>
              <span className="text-[10px] font-mono font-semibold px-2.5 py-1 rounded-lg bg-slate-50 dark:bg-slate-900/50 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 /50">{attachments.length} files</span>
            </div>
            ...
            <div className="space-y-1">
              {attachments.map(att => (
                <div key={att.id} onClick={() => setSelectedPdf(att)} className={`flex items-center justify-between p-3.5 rounded-xl cursor-pointer transition-all duration-200 ${ selectedPdf?.id === att.id 
                      ...
                      : 'hover:bg-slate-100 dark:hover:bg-slate-800 dark:hover:bg-slate-700 border border-transparent'
                  }`}>
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-rose-500/20 to-pink-500/20 flex items-center justify-center shrink-0 border border-rose-500/10">
                      <FileText className="w-5 h-5 text-rose-400" />
                    </div>
                    <div className="min-w-0">
                      <div className="text-sm font-semibold text-slate-900 dark:text-white truncate">{att.fileName}</div>
                      <p className="text-[10px] font-mono text-slate-600 dark:text-slate-300 mt-0.5 flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button onClick={e => { e.stopPropagation(); handleStartEditDoc(att); }} className="btn-ghost text-slate-600 dark:text-slate-300 hover:text-indigo-400"><Pencil className="w-3.5 h-3.5" /></button>
                    <button onClick={e => { e.stopPropagation(); setDeleteConfirm({ id: att.id, name: att.fileName }); }} className="btn-ghost text-slate-600 dark:text-slate-300 hover:text-rose-400"><Trash2 className="w-3.5 h-3.5" /></button>
                  </div>
                </div>
              ))}
        ...
        <div className="lg:col-span-2">
          <div className="rounded-2xl bg-gradient-to-br from-white dark:from-slate-900 to-slate-50 dark:to-slate-800 border border-slate-200 dark:border-slate-700 /50 p-6 shadow-xl min-h-[500px] flex flex-col">
            {selectedPdf ? (
              <>
                <div className="flex items-start justify-between pb-4 border-b border-slate-200 dark:border-slate-700 /50 mb-4">
                  <div className="min-w-0 flex-1">
                    <span className="text-[9px] uppercase font-bold tracking-wider text-slate-600 dark:text-slate-300 ">AI Extraction Result</span>
                ...
                <div className="space-y-3 flex-1 overflow-y-auto">
                  ...
                  <div className="flex items-start gap-3.5 p-4 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 /30">
                    <div className="w-9 h-9 rounded-lg bg-indigo-500/10 flex items-center justify-center shrink-0"><Building className="w-4 h-4 text-indigo-400" /></div>
                    <div><span className="text-[9px] uppercase font-bold text-slate-600 dark:text-slate-300 block mb-0.5">Client</span><span className="text-sm font-semibold text-slate-900 dark:text-white ">{selectedPdf.extractedData?.clientName || 'N/A'}</span></div>
                  </div>
                  {/* Site */}
                  <div className="flex items-start gap-3.5 p-4 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 /30">
                    ...
                    <div><span className="text-[9px] uppercase font-bold text-slate-600 dark:text-slate-300 block mb-0.5">Site</span><span className="text-sm font-semibold text-slate-900 dark:text-white ">{selectedPdf.extractedData?.address || 'N/A'}</span></div>
                  </div>
                  {/* Total Amount */}
                  <div className="flex items-start gap-3.5 p-4 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 /30">
                    ...
                    <div><span className="text-[9px] uppercase font-bold text-slate-600 dark:text-slate-300 block mb-0.5">Total Value</span><span className="text-lg font-bold text-emerald-400">{selectedPdf.extractedData?.totalAmount || 'N/A'}</span></div>
                  </div>
                  ...
                  <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 /30">
                    <span className="text-[9px] uppercase font-bold text-slate-600 dark:text-slate-300 block mb-3">Line Items</span>
                    <ul className="space-y-2">
                      {selectedPdf.extractedData?.items?.map((item, id) => (
                        <li key={id} className="text-[11px] text-slate-600 dark:text-slate-300 flex items-start gap-2.5">
                ...
                {/* Push Button */}
                <button onClick={async () => { try { const { createLiveContact, createLiveDeal } = await import('../lib/api'); if (selectedPdf?.extractedData?.clientName) { await createLiveContact({ name: selectedPdf.extractedData.clientName, company: selectedPdf.extractedData.clientName, email: `${selectedPdf.extractedData.clientName.toLowerCase().replace(/\s/g,'')}@email.com`, phone: '07700 900550', status: 'Lead', revenue: parseInt(selectedPdf.extractedData.totalAmount?.replace(/[^0-9]/g,'') || '0') || 1000, tenant: selectedPdf.extractedData.boilerModel ? 'heating' : selectedPdf.extractedData.screedThickness ? 'screed' : 'electrical' }); await createLiveDeal({ title: `Work Bundle - ${selectedPdf.extractedData.clientName}`, company: selectedPdf.extractedData.clientName, value: parseInt(selectedPdf.extractedData.totalAmount?.replace(/[^0-9]/g,'') || '0') || 1000, stage: 'Lead', tenant: selectedPdf.extractedData.boilerModel ? 'heating' : selectedPdf.extractedData.screedThickness ? 'screed' : 'electrical' }); alert('✅ Pushed to CRM!');
                    }
                  } catch (err) { alert('Error: ' + err); }
                }} className="w-full mt-4 px-5 py-3 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-400 hover:to-purple-500 text-white text-sm font-bold transition-all shadow-lg shadow-indigo-500/20 flex items-center justify-center gap-2">
                  <CheckCircle className="w-4 h-4" /> Push to CRM Pipeline
                </button>

                <div className="text-[10px] text-center pt-4 mt-4 border-t border-slate-200 dark:border-slate-700 /50 font-mono text-slate-600 dark:text-slate-300"> Powered by Gemini 3.5 Flash Extraction
                </div>
              </>
            ) : (
              <div className="flex-1 flex flex-col items-center justify-center text-center">
                <div className="w-16 h-16 rounded-2xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 /30 flex items-center justify-center mb-4">
                  <Eye className="w-7 h-7 text-slate-600 dark:text-slate-300 " />
                </div>
                <span className="text-sm font-medium text-slate-600 dark:text-slate-300 ">Select a document</span>
                <span className="text-[11px] text-slate-600 dark:text-slate-300 mt-1">View parsed extraction data here</span>
              </div>
            )}
          </div>
        </div>
