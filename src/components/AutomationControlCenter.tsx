...
type PlatformBranch = 'facebook' | 'seo_google_ads_wordpress'; export default function AutomationControlCenter() { const [activeTab, setActiveTab] = useState<PlatformBranch>('facebook'); const [workspaceId, setWorkspaceId] = useState('work_luxe_01'); const [campaignName, setCampaignName] = useState(''); const [tenant, setTenant] = useState<'full_home_renovation' | 'kitchen_renovation' | 'bathroom_renovation' | 'granny_flat' | 'extension' | 'multi_unit' | 'new_luxe_homes'>('full_home_renovation');
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setDeleteConfirm(null)}>
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/80 shadow-sm dark:backdrop-blur-md rounded-2xl p-6 max-w-sm w-full mx-4 shadow-2xl animate-scale-in" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-sm text-neutral-600 dark:text-neutral-400 transition-colors mb-6"> Are you sure you want to delete <strong className="text-slate-900 dark:text-white">{deleteConfirm.name}</strong>? This action cannot be undone.
            </p>
            <div className="flex gap-2 justify-end">
              ...
              <button onClick={handleDeleteCampaign} className="px-4 py-2 rounded-xl from-rose-600 to-pink-600 hover:from-rose-500 hover:to-pink-500 text-white text-sm font-bold flex items-center gap-1.5"
              >
      ...
      <div className="relative overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700/80 shadow-sm bg-white dark:bg-slate-900 p-6 backdrop-blur-md shadow-indigo-950/10">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-xs text-neutral-600 dark:text-neutral-400 transition-colors max-w-2xl leading-relaxed"> Synchronize, trigger, and track complex campaigns natively. Select a specialized pipeline branch, formulate targets, and dispatch directly into the n8n automation engine.
            </p>
          </div>
          <div className="flex items-center gap-3">
            <div className="text-right">
              <span className="text-[10px] text-neutral-600 dark:text-neutral-400 transition-colors uppercase tracking-widest block font-mono">Workspace Router</span>
              <select value={workspaceId} onChange={(e) => setWorkspaceId(e.target.value)} className="bg-slate-100 dark:bg-slate-800/70 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white placeholder-slate-400 focus:bg-white dark:focus:bg-slate-800 dark:placeholder-slate-500 text-xs rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-1 focus:ring-black dark:ring-white"
              >
            ...
            <div className="text-right">
              <span className="text-[10px] text-neutral-600 dark:text-neutral-400 transition-colors uppercase tracking-widest block font-mono">Business Division</span>
        ...
        <div className="mt-5 pt-4 border-t border-slate-200 dark:border-slate-700 /80 flex flex-wrap items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-3">
            <span className="text-neutral-600 dark:text-neutral-400 transition-colors font-medium">Pipeline Status:</span>
            <div className={`px-3 py-1 rounded-full border text-[11px] font-semibold flex items-center gap-2 ${ liveStatus === 'Idle' ? 'bg-slate-950/40 text-neutral-600 dark:text-neutral-400 transition-colors border-slate-200 dark:border-slate-700 ' : liveStatus === 'n8n Processing Deep Chains...' ? 'bg-indigo-950/30 text-neutral-500 dark:text-neutral-400 border-indigo-800/50 animate-pulse' : liveStatus.includes('Live') ? 'bg-emerald-950/30 text-emerald-400 border-emerald-800/50' :
          ...
          <div className="flex items-center gap-2 font-mono text-[10px] text-neutral-600 dark:text-neutral-400 transition-colors bg-slate-100 dark:bg-slate-800/50 px-3 py-1 rounded-lg border border-slate-200 dark:border-slate-700 /40">
          ...
          <form onSubmit={handleExecutePipeline} className="relative rounded-2xl border border-slate-200 dark:border-slate-700 /80 bg-white dark:bg-slate-900 p-6 backdrop-blur-xl shadow-xl">
            {isExecuting && (
              <div className="absolute inset-0 bg-slate-100 dark:bg-slate-800/80 rounded-2xl flex flex-col items-center justify-center z-10 backdrop-blur-sm transition-all duration-300">
                ...
                <span className="text-xs font-mono text-neutral-500 dark:text-neutral-400 uppercase tracking-widest animate-pulse">n8n Execution Deep Loop Active</span>
                <span className="text-[10px] text-neutral-600 dark:text-neutral-400 transition-colors mt-1">Pushes live API updates. Waiting for sheets callback...</span>
              </div>
            )}

            {/* Campaign Name Field (Global for all branches) */}
            <div className="mb-5">
              <label className="block text-xs font-bold uppercase tracking-wider text-neutral-600 dark:text-neutral-400 transition-colors mb-1.5"> Campaign / Pipeline Name
              </label>
              <input type="text" value={campaignName} onChange={(e) => setCampaignName(e.target.value)} placeholder="e.g. Q3 High-Efficiency Combi Boiler Drive" className="w-full bg-slate-100 dark:bg-slate-800/70 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white placeholder-slate-400 focus:bg-white dark:focus:bg-slate-800 dark:placeholder-slate-500 text-xs rounded-xl px-3.5 py-2.5 focus:outline-none focus:ring-1 focus:ring-black dark:ring-white focus:border-black dark:border-white transition" required
              />
            </div>
            ...
            {activeTab === 'facebook' && (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-neutral-600 dark:text-neutral-400 transition-colors mb-1.5"> IMAGE / MEDIA URL INPUT
                    </label>
                    <div className="flex items-center gap-2">
                      <input 
                        ...
                        className="w-full bg-slate-100 dark:bg-slate-800/70 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white placeholder-slate-400 focus:bg-white dark:focus:bg-slate-800 dark:placeholder-slate-500 text-xs rounded-xl px-3.5 py-2.5 focus:outline-none focus:ring-1 focus:ring-black dark:ring-white"
                      />
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-neutral-600 dark:text-neutral-400 transition-colors mb-1.5"> Targeted Link Input
                    </label>
                    <input type="text" value={metaTargetLink} onChange={(e) => setMetaTargetLink(e.target.value)} placeholder="https://yourbrand.com/landing" className="w-full bg-slate-100 dark:bg-slate-800/70 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white placeholder-slate-400 focus:bg-white dark:focus:bg-slate-800 dark:placeholder-slate-500 text-xs rounded-xl px-3.5 py-2.5 focus:outline-none focus:ring-1 focus:ring-black dark:ring-white font-mono"
                    />
                  </div>
                </div>
                ...
                {metaMediaUrl && (
                  <div className="border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 p-2.5 rounded-xl text-slate-900 dark:text-white dark:text-emerald-400 flex items-center gap-3">
                    ...
                    />
                    <div className="text-[10px] text-neutral-600 dark:text-neutral-400 transition-colors truncate leading-tight w-full">
                      <span className="font-semibold block text-neutral-600 dark:text-neutral-400 transition-colors mb-1">Live Media Attachment Preview</span>
                ...
                <div className="flex items-center justify-between p-3 rounded-xl border border-slate-200 dark:border-slate-700 /40 bg-slate-100 dark:bg-slate-800/50">
                  <div className="flex flex-col">
                    <span className="text-xs font-semibold text-neutral-600 dark:text-neutral-400 transition-colors">Direct Automation Scheduling</span>
                    <span className="text-[10px] text-neutral-600 dark:text-neutral-400 transition-colors">Wait for custom callback checks before triggering API pushes.</span>
                  </div>
            ...
            {activeTab === 'google_ads' && (
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-neutral-600 dark:text-neutral-400 transition-colors mb-1.5"> Granular Budget (USD Daily)
                    </label>
                    <div className="relative">
                      <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-neutral-600 dark:text-neutral-400 transition-colors">
                        <DollarSign className="w-4 h-4" />
                      </div>
                      <input type="number" value={googleBudget} onChange={(e) => setGoogleBudget(Number(e.target.value))} className="w-full bg-slate-100 dark:bg-slate-800/70 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white placeholder-slate-400 focus:bg-white dark:focus:bg-slate-800 dark:placeholder-slate-500 text-xs rounded-xl pl-8 pr-3.5 py-2.5 focus:outline-none focus:ring-1 focus:ring-black dark:ring-white" min="1"
                      />
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-neutral-600 dark:text-neutral-400 transition-colors mb-1.5"> Target Country / Region Niche
                    </label>
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div>
                    <div className="flex justify-between items-center mb-1.5">
                      <label className="block text-xs font-bold uppercase tracking-wider text-neutral-600 dark:text-neutral-400 transition-colors"> Ad Search Headline
                      </label>
                      <span className={`text-[9px] font-mono ${googleHeadline.length > 30 ? 'text-rose-500' : 'text-neutral-600 dark:text-neutral-400 transition-colors '}`}>
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-1.5">
                      <label className="block text-xs font-bold uppercase tracking-wider text-neutral-600 dark:text-neutral-400 transition-colors"> Ad Long Description
                      </label>
                      <span className={`text-[9px] font-mono ${googleDescription.length > 90 ? 'text-rose-500' : 'text-neutral-600 dark:text-neutral-400 transition-colors '}`}>
                ...
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-neutral-600 dark:text-neutral-400 transition-colors mb-1.5"> Keywords Array Builder
                  </label>
                  ...
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {googleKeywords.map((kw, i) => (
                      <span key={i} className="flex items-center gap-1 bg-slate-50 dark:bg-slate-900/50 text-neutral-600 dark:text-neutral-400 transition-colors text-[10px] px-2 py-0.5 rounded-md border border-slate-200 dark:border-slate-700 /60 font-mono">
                        <span>{kw}</span>
                        <button type="button" onClick={() => removeKeyword(kw)} className="text-neutral-600 dark:text-neutral-400 transition-colors hover:text-black dark:hover:text-white">
                          <X className="w-3 h-3" />
                        </button>
                      </span>
                    ))}
            ...
            {activeTab === 'seo_blog' && (
              <div className="space-y-4">
                <div className="flex p-1 bg-slate-100 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700 /60 w-fit gap-1 mb-2">
                  <button type="button" onClick={() => setSeoPreviewMode('edit')} className={`px-3 py-1 rounded text-[10px] font-semibold transition ${seoPreviewMode === 'edit' ? 'bg-slate-50 dark:bg-slate-900/50 text-neutral-500 dark:text-neutral-400' : 'text-neutral-600 dark:text-neutral-400 transition-colors hover:text-black dark:hover:text-white '}`}
                  > Markdown Editor
                  </button>
                  <button type="button" onClick={() => setSeoPreviewMode('preview')} className={`px-3 py-1 rounded text-[10px] font-semibold transition ${seoPreviewMode === 'preview' ? 'bg-slate-50 dark:bg-slate-900/50 text-neutral-500 dark:text-neutral-400' : 'text-neutral-600 dark:text-neutral-400 transition-colors hover:text-black dark:hover:text-white '}`}
                  > HTML Preview
                  </button>
                </div>
                ...
                {seoPreviewMode === 'edit' ? (
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-bold uppercase tracking-wider text-neutral-600 dark:text-neutral-400 transition-colors mb-1.5"> Article Editorial Title
                        </label>
                      ...
                      <div>
                        <label className="block text-xs font-bold uppercase tracking-wider text-neutral-600 dark:text-neutral-400 transition-colors mb-1.5"> Subtitle / SEO Tagline
                        </label>
                    ...
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-wider text-neutral-600 dark:text-neutral-400 transition-colors mb-1.5"> Meta Excerpt (Short Snippet)
                      </label>
                    ...
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-wider text-neutral-600 dark:text-neutral-400 transition-colors mb-1.5"> Article Body Text (Markdown Ready)
                      </label>
                  ...
                  <div className="border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 p-5 rounded-xl space-y-3 max-h-96 overflow-y-auto">
                    ...
                    <div className="bg-white dark:bg-slate-900 p-2.5 rounded border border-slate-200 dark:border-slate-700 /45 text-neutral-600 dark:text-neutral-400 transition-colors italic text-[11px] leading-relaxed">
                      <strong>Meta Description Excerpt:</strong> {seoExcerpt || 'No excerpt generated.'}
                    </div>
                    <div className="text-xs text-neutral-600 dark:text-neutral-400 transition-colors space-y-2 font-sans pt-2 leading-relaxed">
  ...
  if (para.startsWith('- ')) { return <li key={i} className="ml-4 list-disc text-neutral-600 dark:text-neutral-400 transition-colors">{para.replace('- ', '')}</li>;
                          }
  return para.trim() !== '' ? <p key={i} className="text-neutral-600 dark:text-neutral-400 transition-colors">{para}</p> : null;
                        })
                      ) : (
                        <p className="text-neutral-600 dark:text-neutral-400 transition-colors italic">No article body text written yet.</p>
                      )}
                    </div>
                  </div>
                )}
                ...
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-neutral-600 dark:text-neutral-400 transition-colors mb-1.5"> Meta Tags Multi-Select
                  </label>
                ...
                {/* Cadence & Target */}
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-neutral-600 dark:text-neutral-400 transition-colors mb-1.5"> Automation Firing Cadence
                  </label>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-700/80 shadow-sm dark:backdrop-blur-md bg-white dark:bg-slate-900 p-5 flex flex-col h-72">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-700 /80 pb-3 mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-neutral-600 dark:text-neutral-400 transition-colors">Live Webhook Log Terminal</span>
              </div>
              <button onClick={() => setExecutionLog([])} className="text-[10px] text-neutral-600 dark:text-neutral-400 transition-colors hover:text-black dark:hover:text-white font-semibold"
              > Clear
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto font-mono text-[10px] space-y-2 scrollbar-thin scrollbar-thumb-slate-800">
              {executionLog.map((log, index) => (
                <div key={index} className="flex gap-2 items-start border-l border-slate-200 dark:border-slate-700 /50 pl-2">
                  <span className="text-neutral-600 dark:text-neutral-400 transition-colors flex-shrink-0">{log.time}</span>
                  <span className={`leading-relaxed ${ log.type === 'success' ? 'text-emerald-400' : log.type === 'error' ? 'text-rose-400 font-semibold' : log.type === 'warning' ? 'text-amber-400' : 'text-neutral-600 dark:text-neutral-400 transition-colors '
                  }`}>
                    {log.msg}
                  </span>
                </div>
              ...
              {executionLog.length === 0 && (
                <div className="text-neutral-600 dark:text-neutral-400 transition-colors italic text-center pt-10">Waiting for webhook triggers...</div>
              )}
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-700/80 shadow-sm dark:backdrop-blur-md bg-white dark:bg-slate-900 p-5 flex flex-col">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-700 /80 pb-3 mb-3">
              ...
              <button onClick={loadHistory} className="text-[10px] text-neutral-600 dark:text-neutral-400 transition-colors hover:text-black dark:hover:text-white font-semibold"
              > Refresh
              </button>
            </div>
            ...
            <div className="space-y-2.5 max-h-80 overflow-y-auto">
              {campaignHistory.slice(0, 7).map((c: any) => (
                <div key={c.id} className="p-3 bg-slate-100 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white dark:text-emerald-400 flex flex-col gap-1.5 hover:border-neutral-400 dark:hover:border-neutral-600 transition-colors transition relative group">
                  {/* Hover delete button */}
                  <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button type="button" onClick={(e) => { e.stopPropagation(); setDeleteConfirm({ id: c.id, name: c.title });
                      }} className="p-1 text-neutral-600 dark:text-neutral-400 transition-colors hover:text-rose-400 hover:bg-rose-950/30 rounded border border-transparent hover:border-rose-900/20 transition" title="Delete campaign"
                    >
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] bg-slate-50 dark:bg-slate-900/50 text-neutral-600 dark:text-neutral-400 transition-colors px-1.5 py-0.5 rounded font-bold font-mono uppercase">
                      {c.platform}
                    </span>
                    <span className={`text-[9px] font-semibold px-2 py-0.5 rounded-full ${ c.status.includes('✅') || c.status.includes('Live') ? 'bg-emerald-950/30 text-emerald-400' : c.status.includes('n8n') ? 'bg-indigo-950/30 text-neutral-500 dark:text-neutral-400 animate-pulse' : c.status.includes('❌') ? 'bg-rose-950/30 text-rose-400 border border-rose-900/30' : 'bg-white dark:bg-slate-900 text-neutral-600 dark:text-neutral-400 transition-colors '
                    }`}>
                      {c.status}
                    </span>
                  </div>
                  <div className="text-[11px] font-bold text-neutral-600 dark:text-neutral-400 transition-colors truncate pr-6">
                    {c.title}
                  </div>
                  <div className="flex items-center justify-between text-[9px] text-neutral-600 dark:text-neutral-400 transition-colors">
              ...
              {campaignHistory.length === 0 && (
                <div className="text-neutral-600 dark:text-neutral-400 transition-colors italic text-center py-6 text-xs">No synchronization history found.</div>
              )}
            </div>
          </div>
        </div>
