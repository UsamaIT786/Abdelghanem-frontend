...
export default function LoginScreen({ onLoginSuccess }: LoginScreenProps) { 
  ...
  return (
    <div id="login-container" className="min-h-screen relative flex items-center justify-center bg-[#F8F9FA] overflow-hidden py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className="absolute top-0 left-0 w-full h-96 bg-gradient-to-b from-[#4F46E5]/5 to-transparent pointer-events-none" />
      ...
      <div className="relative w-full max-w-5xl bg-white dark:bg-slate-900/85 backdrop-blur-md rounded-2xl shadow-2xl overflow-hidden border border-white/60 flex flex-col md:flex-row min-h-[600px]">
        ...
        <div className="w-full md:w-5/12 bg-gradient-to-br from-[#1A1A1A] via-[#2D2D2D] to-[#111111] p-8 text-slate-900 dark:text-white flex flex-col justify-between relative">
          ...
          <div>
            <div className="flex items-center gap-2 mb-8">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-r from-[#4F46E5] to-[#06B6D4] flex items-center justify-center text-slate-900 dark:text-white shadow-lg">
          ...
          <div>
            ...
            <div className="mt-4 text-[10px] text-slate-500 dark:text-slate-400 text-center"> All demo accounts password: <code className="text-slate-900 dark:text-white px-1 font-mono">admin123</code>
            </div>
          </div>
        </div>
          ...
          {!showTwoFactor ? (
            <div>
              <div className="mb-6">
                ...
                <p className="text-sm text-slate-600 dark:text-slate-300">Select operational tenant unit to begin workspace dispatch</p>
              </div>
              ...
              <div className="mb-6">
                <label className="text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-widest block mb-2"> Select Associated Tenant Business
                </label>
                <div className="grid grid-cols-2 gap-2">
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-slate-500 dark:text-slate-400">All Modules</div>
                    </div>
                  </button>
              ...
              <form onSubmit={handleSignIn} className="space-y-4">
                ...
                <div className="relative">
                  <User className="absolute left-3.5 top-1/2 transform -translate-y-1/2 w-4.5 h-4.5 text-slate-500 dark:text-slate-400" />
                ...
                <div className="relative">
                  <Lock className="absolute left-3.5 top-1/2 transform -translate-y-1/2 w-4.5 h-4.5 text-slate-500 dark:text-slate-400" />
                  ...
                  />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 focus:outline-none"
                  >
                ...
                <div className="bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl p-3 flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    <button type="button" onClick={triggerMockCaptcha} disabled={captchaVerified || captchaLoading} className={`w-5 h-5 rounded flex items-center justify-center border transition ${ captchaVerified 
                          ...
                          : 'bg-white dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800 dark:hover:bg-slate-700 border-slate-200 dark:border-slate-700'
                      }`}
                    >
                      {captchaLoading ? (
                    ...
                    <div>
                      <span className="text-xs text-slate-600 dark:text-slate-300 font-medium">Verify through hCaptcha secure gate</span>
                      <p className="text-[10px] text-slate-500 dark:text-slate-400">Strict bot and telemetry defense</p>
                    </div>
                  </div>
                  <div className="text-center">
                    <span className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 tracking-wider">hCaptcha</span>
                    <div className="text-[8px] text-slate-500 dark:text-slate-400">Privacy • Terms</div>
                  </div>
                </div>

                {/* Highly polished Sign-In button with hover gradients & active/loading states */}
                <button type="submit" disabled={loading} className="w-full py-3.5 px-4 rounded-xl bg-gradient-to-r from-[#4F46E5] to-[#06B6D4] hover:opacity-95 text-slate-900 dark:text-white font-semibold text-sm shadow-md hover:shadow-[#4F46E5]/10 focus:outline-none focus:ring-2 focus:ring-[#4F46E5] transition flex items-center justify-center gap-2"
                >
                  {loading ? (
                    <>
            ...
            <div>
              <div className="mb-6">
                <div className="w-12 h-12 bg-indigo-50 border border-indigo-200 rounded-full flex items-center justify-center mx-auto mb-4 text-indigo-600 shadow-sm animate-bounce">
                  <Shield className="w-6 h-6" />
                </div>
                <h3 className="text-xl font-bold text-center text-slate-900 dark:text-white">Two-Factor Authentication</h3>
                <p className="text-xs text-center text-slate-600 dark:text-slate-300 mt-1"> Enter the 4-digit code sent to your registered authenticator device configured under Tenant ID.
                </p>
              </div>
              ...
              <form onSubmit={handleVerify2FA} className="space-y-6">
                ...
                <div className="flex gap-2">
                  <button type="button" onClick={() => setShowTwoFactor(false)} className="w-1/2 py-3 px-4 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 font-medium text-xs transition"
                  > Back to Login
                  </button>
                  <button type="submit" className="w-1/2 py-3 px-4 rounded-xl bg-gradient-to-r from-[#4F46E5] to-[#06B6D4] hover:opacity-95 text-slate-900 dark:text-white font-semibold text-xs shadow-md transition flex items-center justify-center gap-1.5"
                  >
